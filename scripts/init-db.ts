...
async function initDatabase() {
  ...
  try {
    ...
    if (error) {
      // If exec_sql RPC doesn't exist, we need to use the SQL editor directly
      console.log('Note: exec_sql RPC not found. Please run the following SQL in Supabase SQL Editor:');
      console.log(createTablesSQL);
      console.log('\nAlternatively, the schema will be created via the ingest script.');
    } else {
      console.log('Tables created successfully!');
    }
  } catch (err) {
    console.log('Creating tables via direct SQL...');
    // Try alternative approach - create via Supabase dashboard or use db push
  }
  ...
  for (const table of tables) {
    ...
    if (error) {
      console.log(`  [!] ${table}: Not yet created (${error.message})`);
    } else {
  ...
  console.log('\n' + '='.repeat(60));
  console.log('If tables are not created, run the SQL manually in Supabase Dashboard');
  console.log('https://supabase.com/dashboard/project/pekacscvibbbfweuwtfe/sql');
  console.log('='.repeat(60));
}
