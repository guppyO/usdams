...
async function getOwnerData(typeSlug: string, page: number, stateFilter?: string) {
  ...
  // Get owner type info
  const { data: ownerType } = await supabase
    .from('owner_types')
    .select('*')
    .eq('slug', typeSlug)
    .single();
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!ownerType) {
    return { title: 'Owner Type Not Found' };
  }

  return {
    title: `${ownerType.name} Owned Dams`,
    description: `Browse ${ownerType.dam_count?.toLocaleString() || 0} dams owned by ${ownerType.name.toLowerCase()} entities across the United States.`,
  };
}

export default async function OwnerTypePage({ params, searchParams }: PageProps) {
  const { type } = await params;
  const { page: pageParam, state } = await searchParams;
  const page = parseInt(pageParam || '1', 10);

  const result = await getOwnerData(type, page, state);

  if (!result) {
    notFound();
  }

  const { ownerType, dams, totalDams, states } = result;
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="container mx-auto px-4">
        <Breadcrumbs items={[
          { label: 'Owner Types', href: '/owner' },
          { label: ownerType.name }
        ]} />
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Browse {totalDams.toLocaleString()} dams owned by {ownerType.name.toLowerCase()} entities across the United States.
          </p>
        </div>
          ...
          <div className="lg:col-span-3">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2">
                {page > 1 && (
                  <Link
                    href={`/owner/${type}?page=${page - 1}${state ? `&state=${encodeURIComponent(state)}` : ''}`}
